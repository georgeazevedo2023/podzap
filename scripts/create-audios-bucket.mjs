#!/usr/bin/env node
/**
 * Idempotently create the `audios` Supabase Storage bucket (Fase 9).
 *
 * Usage:
 *   node --env-file=.env.local scripts/create-audios-bucket.mjs
 *
 * Requires: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 *
 * Notes:
 *   * Separate bucket from `media` — TTS output lives here with a narrower
 *     `allowedMimeTypes` (audio/*) so we can't accidentally upload non-audio
 *     artefacts alongside synthesized podcasts.
