...
- **Webhooks**: `POST /webhook` body `{ url, events: ['messages', 'connection'], enabled: true }` com token de instância. Payload real do `wsmart.uazapi.com` usa shape `{ EventType, instanceName, message: {...} }` (**não** Evolution/Baileys). Schema em `lib/uazapi/types.ts` aceita ambos: UAZAPI-shape primeiro (prod), Evolution-shape fallback (fixtures). Desde 2026-04-25 o parser classifica `text` (incluindo `ExtendedTextMessage`), `audio`, `image` e `video` — extração de URL/mimetype é defensiva (tenta `m.url`, `m.mediaUrl`, `m.audioMessage.url`, etc.) porque a shape exata pra mídia no wsmart ainda não foi capturada. Quando URL não bate, row entra como `media_download_status=skipped` e o worker faz bail clean. **`messages.raw_payload` agora armazena o body HTTP cru** (não o evento Zod-normalizado) pra permitir refino do parser depois sem replay de webhook. Reaction/Sticker/Contact/Document/Poll seguem em `type=other` (preserva `rawType` original).
