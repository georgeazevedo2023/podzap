...
MVP em desenvolvimento. Fases 0–7 concluídas (fundação, auth+multi-tenancy, conexão WhatsApp via UAZAPI, listagem/seleção de grupos, captura de mensagens via webhook, transcrição multimodal via Groq Whisper + Gemini Vision orquestrada pelo Inngest, pipeline `filter → cluster → normalize`, geração do resumo via Gemini 2.5 Pro com tom configurável, tracking de custo em `ai_calls` e saída em `summaries` com status `pending_review`); **Fase 8 (aprovação humana — revisão, edição, aprovar/rejeitar/regenerar com sidebar badge de pendentes) em andamento**. Ver [`ROADMAP.md`](./ROADMAP.md) e [`docs/integrations/approval.md`](./docs/integrations/approval.md).
